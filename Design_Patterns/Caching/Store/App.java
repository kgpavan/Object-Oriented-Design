 ...
 * fast-access storage, and are re-used to avoid having to acquire them again. There are 4 main
 ...
 * written into the DB when the cache is full, 
 * <code>cache-aside</code> which pushes the responsibility of keeping the data synchronized in 
 * both data sources to the application itself.
 ...
 * stores it into the cache for future use. 
 * 
 * <p>
 * These strategies determine when the data in the cache should be written back to the backing 
 * store (i.e. Database) and help keep both data sources synchronized/up-to-date. This pattern 
 * can improve performance and also helps to maintain consistency between data held in the cache 
 * and the data in the underlying data store.
 * </p>
 * 
